...
public:
       ...
       // Initialized at declaration: LastConsOut was read with += before any
       // assignment (bssn_class.C:7295), producing undefined behavior. With
       // gcc -O3 the resulting garbage differed across MPI ranks, causing
       // ranks to disagree on entering Constraint_Out's ghost-zone exchange
       // and deadlocking the collective in MPI_Waitall. LastAnas had the
       // same shape and is initialized here defensively too.
       double LastAnas = 0.0, LastConsOut = 0.0;
       double Courant;
